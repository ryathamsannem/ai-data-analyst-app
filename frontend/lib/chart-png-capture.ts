...
const CHIP_VALUE_FONT_SIZE_PX = 15;
const CHIP_LABEL_FONT_SIZE_PX = 14;
...
function layoutHeaderExport(
  ...
  if (content.title) contentH += 26 + GAP_AFTER_TITLE;
...
async function compositeExportPng(
  ...
  const targetPlotW = Math.round(
    cardInnerW *
      (plotNaturalW < cardInnerW * 0.72 ? PLOT_WIDTH_UTIL_COMPACT : PLOT_WIDTH_UTIL)
  );
...
/** Build export footer — app name plus optional dataset label. */
export function buildPngExportFooterText(datasetName?: string | null): string {
  const base = EXPORT_FOOTER_TEXT;
